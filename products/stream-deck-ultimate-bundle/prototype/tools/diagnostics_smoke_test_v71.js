...
if (manifest.CodePath !== "bin/plugin-v071.cjs") throw new Error(`Expected v0.7.1 CodePath, saw ${manifest.CodePath}`);
...
(async () => {
  ...
  child = spawn(process.execPath, [path.join(pluginDir, manifest.CodePath), "-port", String(port), "-pluginUUID", UUID, "-registerEvent", "registerPlugin"], {
    stdio: ["ignore", "pipe", "pipe"], env: { ...process.env, APPDATA: root, PACKRAT_AUDIO_MOCK: "1", PACKRAT_CONTEXT_MOCK: "1", PACKRAT_DIAGNOSTICS_MOCK: "1", PACKRAT_DIAGNOSTICS_DIR: out }
  });
  ...
  console.log("v0.7.1 diagnostics smoke passed: actual manifest runtime generated secret-free support report through multiplexed action");
