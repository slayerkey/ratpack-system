(() => {
  const FIRST_PORT = 32123;
  const LAST_PORT = 32146;
  ...
  function renderState(state) {
    setDiagnosticStatus("good", `Local diagnostics connected · ${state.listenerPort || "listener"}`);
    ...
    text("diag-log", `Persistent log: ${state.logPath || "%APPDATA%\\PackRat\\CS2CompetitiveDashboard\\logs\\cs2-competitive-dashboard.log"}`);
  ...
  function renderMissing() {
    setDiagnosticStatus("bad", "Local diagnostic service not found");
    text("diag-plugin", "No listener found on PackRat ports");
    ...
    text("diag-log", "Persistent log: %APPDATA%\\PackRat\\CS2CompetitiveDashboard\\logs\\cs2-competitive-dashboard.log");
  ...
  async function copySummary() {
    const summary = currentSummary || [
      ...
      "Persistent log: %APPDATA%\\PackRat\\CS2CompetitiveDashboard\\logs\\cs2-competitive-dashboard.log"
    ].join("\n");
    try {
