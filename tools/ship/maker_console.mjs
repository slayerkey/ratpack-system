 ...
 * Authentication stays local. The SHIP_KIT is generated by GitHub.
 */
...
function inputScore(info, kind) {
  ...
  if (kind === 'icon') {
    ...
    if (/cover|thumbnail|gallery/.test(haystack)) score -= 10;
  } else if (kind === 'cover') {
    if (/cover|thumbnail|hero/.test(haystack)) score += 20;
    if (/app icon|search icon|media-app-icon/.test(haystack)) score -= 20;
    if (info.multiple) score += 1;
    ...
    if (/app icon|search icon|cover|thumbnail/.test(haystack)) score -= 12;
  }
  return score;
}
...
async function chooseFileInput(target, kind, excluded = new Set()) {
  ...
  const best = ranked[0];
  if (kind !== 'gallery' && best.score < 1 && ranked.length === 1) return null;
...
async function setIconAndCover(target) {
  ...
  if (await iconById.count()) {
    ...
    if (!icon) await mediaDiagnostic(target,'icon upload input not found on Maker Console media step');
...
async function run() {
  ...
  await step('7-notes','Verify version and set release notes',async() => {
    ...
    await proseMirror(page,notes,readFileSync(join(KIT,'PASTE_release_notes.txt'),'utf8').trim());
  });
